...
public class LineGraphSeries<E extends DataPointInterface> extends BaseSeries<E> {
    ...
    public void draw(GraphView graphView, Canvas canvas, boolean isSecondScale) {
        ...
        while (values.hasNext()) {
            points.add(new PointF((float) values.next().getX(), (float) values.next().getY()));
        }
        ...
        for (int j = 1; j < points.size(); j++) {
            PointF p = points.get(j);    // current point

            // first control point
            PointF p0 = points.get(j - 1);    // previous point
            float x1 = p0.x + lX;
            float y1 = p0.y + lY;

            // second control point
            PointF p1 = points.get(j + 1 < points.size() ? j + 1 : j);    // next point
